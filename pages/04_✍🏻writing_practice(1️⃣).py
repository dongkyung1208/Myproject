...
st.set_page_config(page_title="Writing Practice (Low)", layout="centered")
...
# ---------------------------
# 세션 상태 초기화 (단계 및 오답 기록용)
# ---------------------------
if "wp_stage" not in st.session_state:
    st.session_state.wp_stage = 1
...
step1_data = [
    {
        "q": "1. This shop mainly sells items (made / making) in Japan.",
        "detail": "This(이) shop(가게는) mainly(주로) sells(판다) items(물건들을) (made:만들어진 / making:만들고있는) in(~에서) Japan.(일본)",
        "trans": "이 가게는 주로 일본에서 만들어진 제품을 판매한다.",
        "options": ["made", "making"],
        "ans": "made",
        "audio_txt": "This shop mainly sells items made in Japan."
    },
    {
        "q": "2. I have a dog (named / naming) Toto.",
        "detail": "I(나는) have(가지고 있다) a(한 마리의) dog(개를) (named:이름 붙여진 / naming:이름 붙이는) Toto.(토토라는)",
        "trans": "나는 토토라는 이름을 가진 개가 있다.",
        "options": ["named", "naming"],
        "ans": "named",
        "audio_txt": "I have a dog named Toto."
    },
    {
        "q": "3. The boy (washes / washing) the dishes is my brother.",
        "detail": "The(그) boy(소년은) (washes:씻는다 / washing:씻고 있는) the(그) dishes(설거지를) is(~이다) my(나의) brother.(남동생)",
        "trans": "설거지를 하고 있는 소년은 내 남동생이다.",
        "options": ["washes", "washing"],
        "ans": "washing",
        "audio_txt": "The boy washing the dishes is my brother."
    }
]
...
# 다시 시작 버튼
if st.button("🔄 처음부터 다시 시작", key="reset_all"):
...
# ==========================================
# STEP 1: 객관식 (한글 뜻풀이 제공)
# ==========================================
if st.session_state.wp_stage == 1:
    st.subheader("📌 STEP 1. 괄호 안에서 알맞은 말을 골라 봅시다.")
    st.info("💡 각 단어 아래의 뜻을 천천히 읽어보고 정답을 선택하세요.")
    
    for i, item in enumerate(step1_data):
        st.markdown(f"**{item['q']}**")
        st.caption(f"📝 {item['detail']}")
        st.write(f"🇰🇷 해석: {item['trans']}")
        st.radio("정답:", item['options'], key=f"s1_q{i}", index=None, horizontal=True)
        st.markdown("---")
        
    if st.button("Step 1 채점하기"):
        for i, item in enumerate(step1_data):
            if st.session_state.get(f"s1_q{i}") != item['ans']:
                st.session_state.wrong_answers.append({"step": 1, "q": item['q'], "ans": item['ans'], "audio_txt": item['audio_txt']})
        st.session_state.wp_stage = 1.5
        st.rerun()

# 1.5 오답 확인 (Step 1)
elif st.session_state.wp_stage == 1.5:
    st.subheader("🚨 Step 1 오답 확인")
    wrongs = [w for w in st.session_state.wrong_answers if w['step'] == 1]
    if not wrongs:
        st.success("대단해요! Step 1을 모두 맞췄습니다. 🎉")
    else:
        st.warning(f"{len(wrongs)}문제를 틀렸습니다. 정답 문장을 듣고 확인해보세요.")
        for w in wrongs:
            st.write(f"❌ 문제: {w['q']}")
...
# ==========================================
    ...
    if st.button("Step 2 채점하기"):
        for i, item in enumerate(step2_data):
            user_ans = st.session_state.get(f"s2_selected_{i}", [])
            if user_ans != item['ans']:
                st.session_state.wrong_answers.append({"step": 2, "q": item['q'], "ans": " ".join(item['ans']), "audio_txt": item['audio_txt']})
        st.session_state.wp_stage = 2.5
        st.rerun()

# 2.5 오답 확인 (Step 2)
elif st.session_state.wp_stage == 2.5:
    st.subheader("🚨 Step 2 오답 확인")
    wrongs = [w for w in st.session_state.wrong_answers if w['step'] == 2]
    if not wrongs:
...
# ==========================================
    ...
    for i, item in enumerate(step3_data):
        ...
        st.text_input("정답 입력:", key=f"s3_q{i}")
        st.markdown("---")
    ...
    if st.button("Step 3 채점하기"):
        for i, item in enumerate(step3_data):
            ...
            if not is_correct:
                ...
                st.session_state.wrong_answers.append({"step": 3, "q": item['q'], "ans": correct_str, "audio_txt": item['audio_txt']})
        st.session_state.wp_stage = 3.5
        st.rerun()

# 3.5 오답 확인 (Step 3)
elif st.session_state.wp_stage == 3.5:
    st.subheader("🚨 Step 3 오답 확인")
    wrongs = [w for w in st.session_state.wrong_answers if w['step'] == 3]
    if not wrongs:
...
# ==========================================
    ...
    for i, item in enumerate(step4_data):
        ...
        st.text_input("올바른 단어 입력:", key=f"s4_q{i}")
        st.markdown("---")
        
    if st.button("최종 제출하기"):
        for i, item in enumerate(step4_data):
            if st.session_state.get(f"s4_q{i}", "").strip().lower() != item['ans']:
                st.session_state.wrong_answers.append({"step": 4, "q": item['q'], "ans": item['ans'], "audio_txt": item['audio_txt']})
        st.session_state.wp_stage = 4.5
        st.rerun()

# ==========================================
# FINAL: 전체 오답 복습 및 HWP 다운로드
# ==========================================
elif st.session_state.wp_stage == 4.5:
